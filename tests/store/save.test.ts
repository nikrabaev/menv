...
test("save+load keep two same-named per-app locals with different values", async () => {
  ...
  const model: RepoModel = {
    ...
    environments: [{ id: "dev", isDefault: true }],
    variables: [
      { id: "var:app:api:NODE_ENV", name: "NODE_ENV", description: "", group: null, secret: false, wiring: [{ consumer: "app:api" }] },
      { id: "var:app:web:NODE_ENV", name: "NODE_ENV", description: "", group: null, secret: false, wiring: [{ consumer: "app:web" }] },
    ],
    consumers: [
